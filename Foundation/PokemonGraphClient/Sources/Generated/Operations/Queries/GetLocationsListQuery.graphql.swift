...
extension GraphClient {
  class GetLocationsListQuery: GraphQLQuery {
    ...
    static let operationDocument: ApolloAPI.OperationDocument = .init(
      definition: .init(
        #"query GetLocationsList($id: Int) { areas: pokemon_v2_location(where: { id: { _eq: $id } }) { __typename ...LocationFramgment } }"#,
    ...
    struct Data: GraphClient.SelectionSet {
      ...
      static var __selections: [ApolloAPI.Selection] { [
        .field("pokemon_v2_location", alias: "areas", [Area].self, arguments: ["where": ["id": ["_eq": .variable("id")]]]),
      ] }
